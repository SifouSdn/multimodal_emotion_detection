...
\begin{figure*}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figures/final_publishable_architecture_tikz.tex}}
...
Calibrated MELD\,$\rightarrow$\,IEMOCAP (prior-shift correction) & 49.91\% & 49.88\% \\
...
The 49.04\% zero-shot result shows that MELD pretraining alone does not transfer reliably to IEMOCAP. A lightweight prior-shift calibration step improves this to 49.91\% weighted F1 (+0.87 points), with paired-bootstrap support ($95\%$ CI $[+0.12,+1.64]$ points; one-sided $p=0.0102$). This domain shift motivated later architecture-level controls for modality alignment, calibration, and robustness rather than relying on retrieval or ensembling alone.
